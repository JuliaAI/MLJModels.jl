...
Import the model implementation code for the model named in the first
argument into the calling module, specfying `pkg` in the case of
duplicate names. Returns the model type.

**Warning** In older versions of MLJ/MLJModels, an *instance* was
returned.
