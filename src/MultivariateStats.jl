...
export RidgeRegressor, PCA, KernelPCA, ICA, LDA, BayesianLDA, BayesianSubspaceLDA, SubspaceLDA, BayesianQDA

import MLJBase
import MLJBase: @mlj_model, metadata_model, metadata_pkg
import StatsBase: proportions, CovarianceEstimator, counts, mean
...
const BayesianQDA_DESCR = "Quadratic Discriminant Analysis: A classifier with a quadratic decision boundary, generated by fitting class conditional densities to the data and using Bayes’ rule for clasification. The model fits a Gaussian density to each class."
...
function MLJBase.fit(model::BayesianLDA, ::Int, X, y)
    ...
    # NOTE: copy/transpose
    Xm_t   = MLJBase.matrix(X, transpose=true) # now p x n matrix
    yplain = MLJBase.int(y) # vector of n Uints in {1,..., nclasses}
    p, n   = size(Xm_t)
    ...
    ## scaled projection_means and core_res.proj by multiplying by sqrt(n - nclasses) this ensures Pᵀ*Σ*P=I
...
* `store_cov=false`: Option to store covariance matrices, one of `true` or `false`.
* `tol = 1e-4`: Threshold used for rank estimation.
* `λ = 0.0` : Regularizes the covariance estimate as `(1-λ) * Σ  + λ * I``   
* `priors=nothing`: if `priors = nothing` estimates the prior probabilities from the data, else it uses the user specified `UnivariateFinite` prior probabilities.

"""

@mlj_model mutable struct BayesianQDA <: MLJBase.Probabilistic
...
struct QDAfitresult{F, T} <: MLJBase.MLJType where T<:Union{MLJBase.CategoricalValue, MLJBase.CategoricalString}
    scalings   :: AbstractArray{F,3} #Vector{Vector{F}} # A 3-dimensional array (p x p x k) of scalings. For each group scalings[:, :, k] transforms observations so that the within group covariance matrix is spherical. 
    priors     :: Vector{F} # A vector containing prior probabilities of each class
    cmeans      :: Matrix{F} # A matrix of dimensions (nclasses, p) containing class means
    class_list :: AbstractVector{T} # A catergorical Array containing unique class members.
    cov        :: Union{Nothing, Vector{Matrix{F}}} # vector of length nclasses containing the (p, p) covariance matrices for each class.
    ldet       :: Vector{F} #Vector of negative half log determinant of the covariance matrix of each class.
end

function MLJBase.fit(model::BayesianQDA, ::Int, X, y)
    features   = MLJBase.schema(X).names
    class_list = MLJBase.classes(y[1]) #class list containing unique entries in y
    nclasses   = length(class_list)

    # NOTE: copy/transpose
    Xm   = MLJBase.matrix(X) # n x p matrix
    yplain = MLJBase.int(y) # vector of n UInts in {1,..., nclasses}
    yplain = Int.(yplain) # converts yplain to a vector of signed Integer type
    n, p   = size(Xm)
    ...
    cweights = counts(yplain)
    if model.priors == nothing
        #priors = proportions(yplain)
        priors = cweights ./ sum(cweights)
    else
        #check if the length of priors is same as nclasses
        length(MLJBase.levels(model.priors)) == nclasses || throw(ArgumentError("Invalid size of `priors`"))
        priors = MLJBase.pdf.(model.priors, class_list)
    ...
    cov = model.store_cov ? cov = [zeros(p, p) for i in 1:nclasses] : nothing  #initilize (p, p, nclasses) multi-dimensional covariance array
    cmeans = zeros(p, nclasses) 
    scalings = zeros(p, p, nclasses)
    ldet = zeros(nclasses) 

    for (k, cweight) in enumerate(cweights)
        cweight > 1 || throw(ArgumentError("The number of observations in group: $(class_list[k]) are too small for QDA"))
        Xg = Xm[ yplain .== k, :] #observations belonging to class k
        cmean = mean(Xg, dims=1)
        cmeans[:, k] = cmean'
        Xgc = Xg .- cmean
        # Xc = U * Diagonal(S) * V' 
        U, S, V = svd(Xgc, full=false) 
        rank = sum(S .> model.tol)
        rank < p && @warn "Variables are collinear"
        S .= (S .^2 ) ./ (cweight - 1) 
        S .= (1 - model.λ) .* S .+ model.λ
        ...
        #Note:: Vt' .* inv.(sqrt.(S)) = Σₖ^(-0.5), where S is the scaling and Vt is the rotation.
        scalings[:, :, k] =  V .* inv.(sqrt.(S))' # assign scaling matrix for class k to scalings matrix
        ldet[k] = -0.5 * sum((log.(S))) # ldet[k] = -0.5*log(|Σₖ|)

    end
    
    cache     = nothing
    report    = NamedTuple{}()
    fitresult = QDAfitresult(scalings, priors, cmeans, class_list, cov, ldet)

    return fitresult, cache, report
end

function MLJBase.fitted_params(::BayesianQDA, fitresult)
    return (class_means       = fitresult.cmeans,
            priors            = fitresult.priors,
            cov               = fitresult.cov)
end

function MLJBase.predict(m::BayesianQDA, fitresult, Xnew)
   
    Xm = MLJBase.matrix(Xnew)
    nclasses = length(fitresult.class_list)
    n = size(Xm, 1)
    P = zeros(nclasses, n) # p x n probability matrix

    for k in 1:nclasses
        scaling = fitresult.scalings[:, :, k] # p x p
        Xc = Xm .- fitresult.cmeans[:, k]' # Xc = (X .−  µₖ)
        Xc .= Xc * scaling #(X .−  µₖ)*Σₖ^(-0.5)
        P[k, :] = -0.5 .* ( sum(Xc .^ 2, dims=2)) # P[k, :] = -0.5*(X .−  µₖ)ᵀΣₖ⁻¹(X .−  µₖ) 
    end

    P .+= fitresult.ldet .+ log.(fitresult.priors) #P[k, :] = -0.5*(X .−  µₖ)ᵀΣₖ⁻¹(X .−  µₖ) -0.5*log(|Σₖ|) + log(πₖ) for every row k in P
    ...
    return [MLJBase.UnivariateFinite(fitresult.class_list, P[:, j]) for j in 1:size(P, 2)]
end
...
metadata_pkg.((RidgeRegressor, PCA, KernelPCA, ICA, LDA, BayesianLDA, SubspaceLDA, BayesianSubspaceLDA, BayesianQDA),
